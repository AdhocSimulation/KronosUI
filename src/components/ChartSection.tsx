...
interface ChartSectionProps {
  ...
  onSignalChange: (signals: Signal[]) => void;
}
...
const ChartSection: React.FC<ChartSectionProps> = ({ colorMode, onStrategyChange, onSignalChange }) => {
  ...
  const handleSignalToggle = (signal: Signal) => {
  ...
  useEffect(() => {
    onSignalChange(selectedSignals);
  }, [selectedSignals, onSignalChange]);
            ...
            {signals.map((signal) => (
              ...
              >
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-xs font-mono ${getSignalTypeColor(signal.type)}`}>
                    {signal.type}
                  </span>
                </div>
              </div>
